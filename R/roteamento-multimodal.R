#' Padroniza um ponto de entrada (`sf`/`sfc` POINT ou `c(lon, lat)`) como sf de 1 feature
#' @keywords internal
#' @noRd
.padroniza_ponto <- function(p) {

  if (inherits(p, "sf")) {
    if (nrow(p) != 1) stop("`origem`/`destino` devem ter exatamente 1 feature.", call. = FALSE)
    if (is.na(sf::st_crs(p))) sf::st_crs(p) <- 4326
    return(sf::st_sf(geometry = sf::st_geometry(p)))
  }

  if (inherits(p, "sfc")) {
    if (length(p) != 1) stop("`origem`/`destino` devem ter exatamente 1 feature.", call. = FALSE)
    if (is.na(sf::st_crs(p))) sf::st_crs(p) <- 4326
  ...
  stop("`origem`/`destino` devem ser um objeto sf/sfc POINT (1 feature) ou c(lon, lat).", call. = FALSE)
}
